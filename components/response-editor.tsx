...
export function ResponseEditor({ responseBody, onResponseChange, onTest }: ResponseEditorProps) {
  ...
  const insertFakerTag = (tag: string) => {
    ...
    const cursorPosition = (document.activeElement as HTMLTextAreaElement)?.selectionStart || currentBody.length
    const beforeCursor = currentBody.substring(0, cursorPosition)
    const afterCursor = currentBody.substring(cursorPosition)
  ...
  const getActiveDynamicFeatures = () => {
    const features = []
    // Add null check for responseBody
    if (responseBody && responseBody.includes("<<") && responseBody.includes(">>")) {
